...
const Dashboard: React.FC = () => {
  const { repositories, findings, scans, isScanning, activeScanRepo, triggerScan } = useSecurity();
  const navigate = useNavigate();

  const avgScore = repositories.length
    ? Math.round(repositories.reduce((a, r) => a + r.trustScore, 0) / repositories.length)
    : 82;
  ...
  return (
    <div className="flex-1">
      ...
      <div className="mb-xl">
        ...
        <p className="text-xs text-on-surface-variant mt-1">Here's what's happening with your repositories.</p>
      </div>
      ...
      <div className="grid grid-cols-12 gap-gutter">
        ...
        <div className="col-span-12 lg:col-span-6 glass-card rounded-xl p-lg flex flex-col justify-between">
          ...
          <div className="flex flex-col md:flex-row items-center gap-xl">
            <TrustGauge score={avgScore} />
            <div className="flex-1 w-full space-y-4">
              {[
                { label: 'Security', val: 85 },
                { label: 'Dependencies', val: 80 },
                { label: 'Secrets', val: 90 },
                { label: 'Code Quality', val: 72 }
              ].map(item => (
                <div key={item.label} className="flex items-center justify-between">
                  ...
                  <div className="w-2/3 flex items-center gap-sm">
                    <div className="flex-1 bg-surface-container-highest h-1.5 rounded-full overflow-hidden">
                      <div className="bg-secondary h-full rounded-full transition-all" style={{ width: `${item.val}%` }} />
                    </div>
                    <span className="font-code-sm text-[12px] w-12 text-right text-on-surface-variant">{item.val}/100</span>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div key={repo.id} className="flex items-center justify-between p-sm bg-surface-container-high/40 rounded-lg border border-outline-variant/20 hover:border-primary/20 transition-all group">
                <div className="flex items-center gap-md">
                  <div className="w-8 h-8 rounded bg-surface-container flex items-center justify-center border border-outline-variant/50">
                    <span className="material-symbols-outlined text-primary text-[18px]">{repo.icon}</span>
                  </div>
                  <div>
                    <p className="font-bold text-sm text-on-background group-hover:text-primary transition-colors">{repo.name}</p>
                    <p className="text-[11px] text-outline font-code-sm">{repo.branch}</p>
                  </div>
                </div>
                <div className="flex items-center gap-md">
                  {activeScanRepo === repo.id ? (
                    ...
                    <span className={`font-bold text-sm ${repo.trustScore >= 80 ? 'text-secondary' : repo.trustScore >= 60 ? 'text-tertiary' : 'text-error'}`}>
                      {repo.trustScore}/100
                    </span>
                  )}
                  <button
        ...
        {[
          ...
          { label: 'Fixed', value: 892, icon: 'check_circle', color: 'text-secondary', borderColor: '' }
        ].map(stat => (
          <div key={stat.label} className={`col-span-12 sm:col-span-6 lg:col-span-3 glass-card p-lg rounded-xl flex flex-col gap-sm ${stat.borderColor}`}>
            ...
            <div className="text-[40px] font-bold text-on-background leading-none">{stat.value.toLocaleString()}</div>
            <div className="text-xs text-on-surface-variant">
              {stat.label === 'Critical' ? 'Requires immediate action' : stat.label === 'Total Findings' ? 'Across all repos' : 'All time resolution'}
            </div>
          </div>
        ))}
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-outline-variant/30">
              {scans.slice(0, 5).map(scan => (
                <tr key={scan.id} className="hover:bg-surface-container/50 transition-colors cursor-pointer" onClick={() => navigate(`/scans/${scan.id}`)}>
                  <td className="py-3 px-lg font-bold text-sm text-on-background hover:text-primary transition-colors">{scan.repoName}</td>
                  <td className="py-3 px-lg">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold flex items-center gap-1 w-fit ${
                      scan.status === 'COMPLETED' ? 'bg-secondary/10 text-secondary border border-secondary/20' :
                      scan.status === 'RUNNING' ? 'bg-primary/10 text-primary border border-primary/20' :
                      'bg-error/10 text-error border border-error/20'
                    }`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${scan.status === 'COMPLETED' ? 'bg-secondary' : scan.status === 'RUNNING' ? 'bg-primary animate-pulse' : 'bg-error'}`} />
                      {scan.status}
                    </span>
                  </td>
                  <td className="py-3 px-lg">
                    <span className={`font-bold text-sm font-code-sm ${scan.trustScore >= 80 ? 'text-secondary' : scan.trustScore >= 60 ? 'text-tertiary' : 'text-error'}`}>
                      {scan.trustScore}/100
                    </span>
                  </td>
                  <td className="py-3 px-lg text-on-surface-variant text-sm">{scan.scannedAt}</td>
                </tr>
              ))}
            </tbody>
          </table>
